...
extern int elevator_up,elevator_down,elevator_select,elevator_moving,elevator_reserved,elevator_alarm,floor_sorg,floor_dest,position,elevator_emergency,potential_alarm,elevator_maintenance,modTone,nbutton;
...
nbutton -> which button is pressed on the screen: 
													1: SAVE
													2: QUIT
													3: SELECT 1
													4: SELECT 2
													
modTone -> flag: note modification mode is on
note1/note2-> indexes for access frequency vector vettK (for first note/second note)
*/
...
void TIMER0_IRQHandler (void)
{
	...
	if(elevator_alarm==0){
	if(elevator_down==1 && position>0){
		position--;
		}
		else if(elevator_up==1 && position<80){
		position++;
		}
	...
	if(position==floor_dest){
		if(elevator_reserved==1 && elevator_select==0){
			...
			disable_timer(3);
			init_timer(3,0x002625A0);							// t=0.1s, f=25000000 --> cc=0.1*25000000=2500000 ->  0x002625A0	BLINK 5Hz end of moving
			reset_timer(3);
			enable_timer(3);
			floor_sorg=position;
			...
			//end user moving --> disable all timers and clear all flags
			disable_timer(0);
			reset_timer(0);
			disable_timer(2);
			reset_timer(2);
			...
			elevator_down=0;
			disable_timer(3);
			enable_timer(3); //blink of end moving
			disable_timer(1);
			reset_timer(1);
...
void TIMER1_IRQHandler (void)
{
	...
	//60s expired to activate elevator -> undo reservation
		...
		j++;
			if(j==6){ //timer: 50 ms for 6 times (for selection of button on touch screen with different colors)
			j=0;
			if(modTone==0 && nbutton!=0 && elevator_maintenance==1){ //if not in modify note mode and if pressure of a button --> deselection
			ButtonDeselection();
			nbutton=0;
				}
			}
...
//frequency vector
	 ...
	 */	
	static int i3=0;
			//blinking+enable speaker if emergency mode is on
	if(i3%2==0){
		LED_On(7);
		if(elevator_emergency==1){
