...
extern int note1; //indice per accedere al vettore delle frequenze, inizializzato ad A (indice 3)
extern int note2;
...
void ADC_IRQHandler(void) {
  ...
  if(modTone==1){
	...
	if(AD_current != AD_last){
		...
		if(nTone==3){
				note1=AD_current*12/0xFFF; //scelta della prima nota attraverso scelta dell'indice "note1"
				GUI_Text(77,100, (uint8_t *) NameNote[13],Blue,White);
				GUI_Text(77,100, (uint8_t *) NameNote[note1],Blue,White);
		}
		if(nTone==4){
				note2=AD_current*12/0xFFF; //scelta della seconda nota attraverso scelta dell'indice "note2"
