...
loop: l.d F1, v1(R1) ;  FDEMW +1
      ...
      l.d F4, v4(R1) ;      FDEMW
      l.d F7, v3(R1) ;       FDEMW
      add.d F5,F8,F7 ;        FDSSSSSEEEMW +3 (+6)
      mul.d F9,F3,F4 ;         FDEEEEEEEEMW +1       INIZIA SUBITO perchè sono vuoti gli stadi della pipeline
      div.d F6,F9,F5 ;          FDDDDDSSSEEEEEEEEEEEEEEEEEEEEEEEEMW +24
      s.d F5,v5(R1) ;            FDSSSSSSESMW
      s.d F6,v6(R1) ;             FSSSSDSSSSSSSSSSSSSSSSSSSSSSSSSEMW +1
      daddui R1,R1,8 ;                     FFFFDSSSSSSSSSSSSSSSSSSEMW +1
bne R1,R2,loop ;                                                 FDDEMW  +2   (stallo nella fase di decode perchè R1 non è ancora scritto (read after write- raw))        
halt ;                                                                     +1 (*30 volte)

;6+(30*44)=1326
; da exec --> 1326
; si ha un miglioramento generale di soli 2 cc a ciclo invece che di 3 --> questo fa sì che il risultato sperimentale sia diverso rispetto al valore calcolato con la legge di amdhal