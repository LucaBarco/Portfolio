	...
	while (in != 'E') {
		...
		case 'R':
			...
			if (FilePos.QuadPart < lpFileSize->QuadPart) {
				...
				//Locking the record
				if (LockFileEx(hIn, LOCKFILE_EXCLUSIVE_LOCK, 0, FilePos.LowPart, FilePos.HighPart, &ov)) {
					ReadFile(hIn, &tmp, sizeof(struct mys), &nRd, &ov);
					_tprintf(_T("%d %d %s %s %d\n"), tmp.id, tmp.rn, tmp.n, tmp.s, tmp.mark);
					UnlockFileEx(hIn, 0, FilePos.LowPart, FilePos.HighPart, &ov);
				}
				else {
					_tprintf(_T("Error in locking file!\n"));
				}

			}
			else {
				_tprintf(_T("id not present!\n"));
			}
			break;
		...
		case 'W':
			...
			if (LockFileEx(hIn, LOCKFILE_EXCLUSIVE_LOCK, 0, FilePos.LowPart, FilePos.HighPart, &ov)) {
				if (FilePos.QuadPart < lpFileSize->QuadPart) {
					...
					_tprintf(_T("--> WRITING NEW RECORD!\n"));
				}
				if (!WriteFile(hIn, &myse, sizeof(struct mys), &nWrt, &ov)) {
