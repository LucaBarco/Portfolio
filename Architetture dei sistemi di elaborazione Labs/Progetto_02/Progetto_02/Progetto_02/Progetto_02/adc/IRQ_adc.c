...
void ADC_IRQHandler(void) {
  ...
  if(modTone==1){
	...
	if(AD_current != AD_last){
		...
		if(nTone==3){
				note1=AD_current*12/0xFFF; //scelta della nota attraverso scelta dell'indice "note"
				GUI_Text(77,100, (uint8_t *) NameNote[13],Blue,White);
				GUI_Text(77,100, (uint8_t *) NameNote[note1],Blue,White);
		}
		if(nTone==4){
				note2=AD_current*12/0xFFF; //scelta della nota attraverso scelta dell'indice "note"
