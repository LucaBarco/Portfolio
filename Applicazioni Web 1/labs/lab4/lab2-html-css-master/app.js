...
class App{
    ...
    addTask= function(pars){
        ...
        this.i=this.i+1;
        this.tasklist.push(new Task(pars,this.i));
    }
    ...
    let d=document.createTextNode(task.deadline.format('MMMM Do YYYY, h:mm:ss a'));
    let date=document.createElement('div');
    date.className='date';
    date.append(d);
    if(task.deadline.diff(moment())<0){
...
//check if there are a new project
    ...
    appendNewTask=function(task){
        ...
        const newElement = this.createNewTask(task);
            ...
            if(!Number.isNaN(task.deadline.valueOf())){
                const now=new Date();
                console.log("I will set a time out for task"+task.id+" value: "+task.deadline.diff(moment()));
                setTimeout(function() {
                    const li = document.getElementById("task" + task.id);
                    const date = li.getElementsByClassName("date")[0];
                    date.classList.add('expired');
                }, task.deadline.diff(moment()), task);
            }
...
document.getElementById('new-task').addEventListener('submit',event=>{   
                                                                    ...
                                                                    if(d!=undefined && p != undefined && dd!=undefined){
                                                                        application.addTask(new Array(p,d,ur,sh,dd));
